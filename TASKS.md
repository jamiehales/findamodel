...
* Fix inconsistent offsets in model data - instead of directly returning the stl file, have the backend transform the vertices in the and return those
...
* The API call from model to the frontend should contain the calculated sphere for the model bounds, and it should pre-transform the vertices by this - making the center of the vertices on the xz plane

* Add the following field to the model metadata - size (x/y/z dimensions of the object in mm), and spherical center of the model. Making sure to use the model service for both of these tasks so that the data is pre-transformed into a y up coordinate system
